...
const STATUS_MAP: Record<string, { label: string; icon: React.ReactNode; className: string }> = {
  ...
  aguardando_aceite: { label: "Aguardando Cliente", icon: <UserCheck className="h-4 w-4" />, className: "bg-purple-100 text-purple-700" },
...
export default function DesvioDetalhe() {
  ...
  const getAvailableStatuses = () => {
    ...
    if (isPunchList) {
      statuses.push({ value: "aguardando_aceite", label: "Aguardando Cliente", icon: STATUS_MAP.aguardando_aceite.icon });
    }
